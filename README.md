...
| 緑の帯（利益があるときだけ） | 「BTC/USDT　OKXで買い → Binanceで売り　+1.23 USDT」のように、ペア・方向・利益 |
| 取引金額 | いくらぶん売買するか（既定 100 USDT）。式も履歴もこの金額で計算する |
| 通貨ペアの枠（PC では 2 列） | 下のスケッチ。見出しの右に「利益あり / 利益なし / データ待ち」 |
...
BTC/USDT                                        利益なし
              ...
              Binanceで買い → OKXで売り
         -0.0954   −   0.1999   =   -0.2953
          価格差       手数料        差引
              0.00164 BTC ≈ 100.00 USDT

取引所      買値 (ask)          売値 (bid)         更新
Binance     61,000.1 [買]       60,999.9           1秒前
OKX         61,000.5            61,000.0 [売]      1秒前
```

- **方向と式**: より有利な方向と、取引金額ぶんの「価格差 − 手数料 ＝ 差引」。差引がマイナスなら、その大きさが利益までの距離
- **数量**: 取引金額 ÷ 買値。板で利益が出る量が足りないときは、その量で計算してその旨を添える
- **価格の表**: 各取引所の買値・売値と、最後に更新されてからの時間。使う 2 つの価格（買う取引所の買値、売る取引所の売値）に色と「買」「売」の札が付く
...
- **タブのタイトル**: 利益が出ている間は `● BTC +1.23` のような目印が付く
...
1. 差引がプラスの間、買値は安い順・売値は高い順に板を突き合わせて数量を積み上げる
2. 純利益 = 売った代金 − 買った代金 − 両方の手数料
3. 数量に固定の上限・下限はない。受け取った板の段数が事実上の上限で、板を使い切ったら「実際はもっと多い可能性がある」と表示する

手数料は、買いは代金に上乗せ、売りは受取から差し引いて USDT にそろえます。率は取引所ごとに設定の `takerFeeRate` で決まり、サーバー全体に共通です（閲覧者ごとには変えられません）。

同じ取引所では売値 < 買値なので、両方向が同時に利益になることはありません。
