---
name: add-pair
description: 監視する通貨ペアの追加・削除時に、設定を変更し、両取引所の購読と配信を確認する。
---
...
2. `backend/config.json` の `pairs` の末尾に足す（減らすときは要素を消す）。並びは画面の初期表示順。重複や形式の誤りは起動時のエラーで止まる。
...
4. 下の合格条件 (a) (b) が通るまで確かめる。ブラウザのツールがあれば (c) も見る。失敗した場合は原因を確認する。取引所やネットワークの問題なら同じ編集・起動を繰り返さず、確認できなかった範囲を報告する。
5. 片方の取引所にしか無いペアは、カードが「データ待ち」のまま判定されない。(a) で購読の拒否が出るか、(c) で気配が片方しか入らなければ、足さずにその旨をユーザーに伝える。対応が不明な場合は、ユーザーに確認してから進める。
6. コミットを依頼された場合は 1 つにまとめる。メッセージは `config: 通貨ペアに ADA/USDT を追加`（減らすなら `を外す`）。PR を頼まれたら本文に合格条件の結果を書く。
...
設定の妥当性と購読の可否は起動・配信で確認する。`make test` だけで購読確認の代わりにしない。
