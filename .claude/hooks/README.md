...
- PostToolUse は編集対象の backend / frontend を確認し、Stop は Git の変更一覧から確認する。シェルでの編集は Stop で拾う。
- Docker が停止していればスキップする。lint 失敗は終了コード 2 で返す。Stop による再開中は `stop_hook_active` を確認し、無限ループを防ぐ。
- ホストに Python 3・jq・Git・make・Docker Compose が必要。Go / Node.js の整形・lint は既存の Makefile に従ってコンテナ内で実行する。
